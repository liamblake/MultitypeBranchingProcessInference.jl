...
function read_datasets(filenames, datasetnames, applyrounding, nburnin, paramnames)
    ...
    for filename in filenames
        ...
        println("size: $(size(samples)). paramnames: $(paramnames)")
...
"""
    makertplot(chains, paramnames)

Create a plot comparing the Rt samples to the true Rt (specified in
config and used to generate data).
"""
function makeRtplot(times, Rt, chains)
    p = plot()

    plot!(p, times, Rt; label="Simulated", color=:black, linestyle=:solid, linewidth=2, seriestype=:steppost)

    for (datasetname, chain) in chains
        errorline!(1:10, chain[:R_0], errorstyle=:plume, label=datasetname, color=cmap(1))
    end

    plot!(xlabel=L"Day $t$", ylabel=L"$R_t$")
    return p
end
...
function main(argv)
    ...
    Rtplot = makeRtplot(model_timestamps, model_Rt, chains)
    ...
    # Save Rt plot - one plot for all simulations
    figfilename = joinpath(pwd(), "figs", "Rtplot_$(caseidentifier).$(FIGURE_FILE_EXT)")
    savefig(Rtplot, figfilename)
